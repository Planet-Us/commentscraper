...
// const server = app.listen(process.env.PORT || '3016', () => {
//     console.log('server listening on port %s', server.address().port);
// });
exports.getComments = functions.https.onRequest(async (request, response) => {
    console.log(request.query);
    response.set('Access-Control-Allow-Origin', '*');
    const result = await getYTComment(request.query.searchText);
    response.send(result);
});

exports.getCommentTemp = functions.https.onRequest(async (request, response) => {
    console.log(request.query);
    response.set('Access-Control-Allow-Origin', '*');
    const result = await scrapeCommentsOnlyTen(request.query.searchText);
    response.send(result);
});
exports.getComment = functions.https.onRequest(async (request, response) => {
    console.log(request.query);
    response.set('Access-Control-Allow-Origin', '*');
    const result = await scrapeComments(request.query.searchText);
    response.send(result);
});
